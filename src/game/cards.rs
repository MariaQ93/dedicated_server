...
// Compare who won and return the index of the winner
pub fn card_function(community_query: &Query<&CommunityCards>, players: &[&Player]) -> Vec<usize> {
    ...
    let mut max_hand=Hand::_new_blank();
    let mut max_index=vec![0];
    for (index,player_cards_component) in players.iter().enumerate() {
        ...
        if player_cards.len() + community_cards.len() >= 5 {
            ...
            if max_hand.is_empty(){
                max_hand=hand;
            }else{
                let comparison = compare_hands(&mut max_hand.clone(), &mut hand.clone());
